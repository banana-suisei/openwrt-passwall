						...
						users = {
							{
								...
								flow = (node.protocol == "vless" and (encryption ~= "none" or node.tls == "1") and (node.transport == "raw" or node.transport == "tcp" or node.transport == "xhttp") and node.flow and node.flow ~= "") and node.flow or nil
	...
	if node.protocol == "vmess" or node.protocol == "vless" then
		if node.uuid then
			...
			for i = 1, #node.uuid do
				clients[i] = {
					id = node.uuid[i],
					flow = (node.protocol == "vless" and (encryption ~= "none" or node.tls == "1") and (node.transport == "raw" or node.transport == "xhttp") and node.flow and node.flow ~= "") and node.flow or nil
